# vLLM-Ascend Expert Offload — Code Study & Reference (`moe_offload_v2.0`)
...
## 11. Next steps
See `expert-offload-optimization-ideas.md` for the prioritized, designed optimization hypotheses and the experiments to run first. The single highest-signal first move: instrument path-share (cache vs pool), per-layer next-step hit rate, and copy counts on the existing `--enforce_eager` inline path — those three numbers tell you where the time goes and whether prefetch is even viable.
